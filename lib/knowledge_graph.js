...
export async function getEntity(id) {
  const db = await getDb();
  const row = db.get('SELECT * FROM entities WHERE id = ?', id);
  if (!row) return null;
...
export async function getGraphStats() {
  const db = await getDb();
  const entities = db.get('SELECT COUNT(*) as n FROM entities');
  const edges = db.get('SELECT COUNT(*) as n FROM edges');
  const types = db.all('SELECT type, COUNT(*) as count FROM entities GROUP BY type ORDER BY count DESC');
  const relations = db.all('SELECT relation, COUNT(*) as count FROM edges GROUP BY relation ORDER BY count DESC LIMIT 10');
  return {
...
export async function clearGraph() {
  ...
  db.run('DELETE FROM media_nodes');
...
export async function exportGraphForVisualization() {
  const db = await getDb();
  const entities = db.all('SELECT id, name, type, description FROM entities');
  const edges = db.all('SELECT source_id, target_id, relation, weight FROM edges');
