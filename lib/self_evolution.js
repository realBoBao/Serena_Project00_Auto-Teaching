...
export async function evaluateResponse(query, response, context = {}) {
  ...
  persistEvaluation(evaluation).catch(() => {});
...
// Lưu AB test data và evaluation log vào SQLite để persist giữa các lần chạy

import sqlite3 from 'sqlite3';
import { open } from 'sqlite';
import path from 'path';

const EVOLUTION_DB_PATH = path.resolve('./data/self_evolution.db');
let evoDbPromise = null;

async function getEvoDb() {
  if (evoDbPromise) return evoDbPromise;
  evoDbPromise = (async () => {
    const db = await open({ filename: EVOLUTION_DB_PATH, driver: sqlite3.default.Database });
    await db.exec(`
      ...
      CREATE INDEX IF NOT EXISTS idx_eval_low_quality ON evaluation_log(is_low_quality);
      CREATE INDEX IF NOT EXISTS idx_eval_created ON evaluation_log(created_at);
    `);
    return db;
  })();
  return evoDbPromise;
...
/** Lưu AB test data vào SQLite */
export async function persistABTests() {
  try {
    const db = await getEvoDb();
    for (const [testId, testData] of abTests) {
      await db.run(
        'INSERT OR REPLACE INTO ab_tests (test_id, data, updated_at) VALUES (?, ?, datetime("now"))',
        [testId, JSON.stringify(testData)]
      );
    }
    logger.info(`[SelfEvolution] Persisted ${abTests.size} AB tests`);
  } catch (err) {
    logger.warn('[SelfEvolution] persistABTests failed:', err.message);
  }
}

/** Load AB test data từ SQLite */
export async function loadPersistedABTests() {
  try {
    const db = await getEvoDb();
    const rows = await db.all('SELECT test_id, data FROM ab_tests');
    for (const row of rows) {
      try {
        const data = JSON.parse(row.data);
        abTests.set(row.testId, data);
      } catch { /* skip corrupt */ }
    }
    if (rows.length > 0) {
      logger.info(`[SelfEvolution] Loaded ${rows.length} persisted AB tests`);
    }
  } catch (err) {
    logger.warn('[SelfEvolution] loadPersistedABTests failed:', err.message);
  }
}

/** Lưu evaluation vào SQLite */
export async function persistEvaluation(evaluation) {
  try {
    const db = await getEvoDb();
    await db.run(
      'INSERT INTO evaluation_log (query_preview, score, is_low_quality, checks) VALUES (?, ?, ?, ?)',
      [
        evaluation.query?.slice(0, 100) || '',
        evaluation.score || 0,
        evaluation.lowQuality ? 1 : 0,
        JSON.stringify(evaluation.checks || []),
      ]
    );
  } catch (err) {
    logger.warn('[SelfEvolution] persistEvaluation failed:', err.message);
  }
}

/** Cleanup old evaluations (giữ 30 ngày) */
export async function cleanupOldEvaluations(daysToKeep = 30) {
  try {
    const db = await getEvoDb();
    const result = await db.run(
      "DELETE FROM evaluation_log WHERE created_at < datetime('now', ?)",
      [`-${daysToKeep} days`]
    );
    if (result?.changes > 0) {
      logger.info(`[SelfEvolution] Cleaned up ${result.changes} old evaluations`);
    }
  } catch (err) {
    logger.warn('[SelfEvolution] cleanupOldEvaluations failed:', err.message);
  }
}

// Auto-load persisted data on import
loadPersistedABTests().catch(() => {});
