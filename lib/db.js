...
export async function runQuery(sql, params = []) {
  if (typeof sql !== 'string') {
    logger.error(`[DB] runQuery called with non-string sql: ${JSON.stringify(sql)}`);
    throw new Error(`runQuery: sql must be a string, got ${typeof sql}`);
  }
  const db = await getDb();
  try {
...
export async function getOne(sql, params = []) {
  ...
  try {
    ...
    logger.error(`[DB] getOne failed: ${String(sql).slice(0, 80)}...`, err.message);
    throw err;
  }
}
