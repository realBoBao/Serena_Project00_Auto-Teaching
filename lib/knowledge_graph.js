...
import { openDbFile } from './sqlite_adapter.js';
...
export async function getDb() {
  ...
  dbPromise = (async () => {
    const db = await openDbFile(KG_DB);
    ...
    // ponytail: node:sqlite exec() only runs first statement — split into separate calls
    await db.exec(`CREATE TABLE IF NOT EXISTS entities (
      id TEXT PRIMARY KEY, name TEXT NOT NULL, type TEXT DEFAULT 'concept',
      description TEXT DEFAULT '', domain TEXT DEFAULT 'general',
    ...
    await db.exec(`CREATE TABLE IF NOT EXISTS entity_aliases (
      alias TEXT PRIMARY KEY, entity_id TEXT NOT NULL,
      FOREIGN KEY (entity_id) REFERENCES entities(id)
    )`);
    await db.exec(`CREATE TABLE IF NOT EXISTS media_nodes (
    ...
    await db.exec('CREATE INDEX IF NOT EXISTS idx_entities_name ON entities(name)');
    await db.exec('CREATE INDEX IF NOT EXISTS idx_entities_type ON entities(type)');
    await db.exec('CREATE INDEX IF NOT EXISTS idx_entities_domain ON entities(domain)');
    await db.exec('CREATE INDEX IF NOT EXISTS idx_edges_source ON edges(source_id)');
    await db.exec('CREATE INDEX IF NOT EXISTS idx_edges_target ON edges(target_id)');
    await db.exec('CREATE INDEX IF NOT EXISTS idx_edges_relation ON edges(relation)');
    await db.exec('CREATE INDEX IF NOT EXISTS idx_aliases_entity ON entity_aliases(entity_id)');
    await db.exec('CREATE INDEX IF NOT EXISTS idx_media_entity ON media_nodes(entity_id)');
    await db.exec('CREATE INDEX IF NOT EXISTS idx_media_type ON media_nodes(media_type)');
    return db;
  })();
  return dbPromise;
}
...
export async function clearGraph() {
  const db = await getDb();
  // Ensure tables exist before DELETE (idempotent)
  try { db.prepare('DELETE FROM edges').run(); } catch { /* table may not exist */ }
  try { db.prepare('DELETE FROM entity_aliases').run(); } catch { /* table may not exist */ }
  try { db.prepare('DELETE FROM media_nodes').run(); } catch { /* table may not exist */ }
  try { db.prepare('DELETE FROM entities').run(); } catch { /* table may not exist */ }
}
