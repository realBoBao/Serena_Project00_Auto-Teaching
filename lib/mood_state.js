...
class MoodStateManager {
  ...
  async getDominantMood(userId, days = 7) {
    ...
    const rows = db.all(`SELECT state, COUNT(*) as count FROM mood_history WHERE user_id = ? AND recorded_at > datetime('now', ?) GROUP BY state ORDER BY count DESC LIMIT 1`, [userId, `-${days} days`]);
  ...
  async cleanup(daysToKeep = 90) {
    const db = await getDb();
    if (!db) return;
    db.run("DELETE FROM mood_history WHERE recorded_at < datetime('now', ?)", [`-${daysToKeep} days`]);
    db.run("DELETE FROM mood_transitions WHERE recorded_at < datetime('now', ?)", [`-${daysToKeep} days`]);
