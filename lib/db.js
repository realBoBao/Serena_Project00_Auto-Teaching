...
/**
 * Check if source needs update (hash changed) or is new.
 * @returns {Object} — { exists: boolean, needsUpdate: boolean }
 */
export function checkSourceStatus(id, newHash) {
  const db = loadDb();
  const existing = db[id];
  if (!existing) return { exists: false, needsUpdate: true };
  if (newHash && existing.hash !== newHash) return { exists: true, needsUpdate: true };
