...
// ── User Preferences ──
// In-memory cache + SQLite persistence (via user_profile.js)
...
export function setUserPreference(userId, prefs) {
  ...
  const updated = {
    preferredModel: prefs.preferredModel || current.preferredModel || 'auto',
    preferredSources: prefs.preferredSources || current.preferredSources || [],
    learningEnabled: prefs.learningEnabled !== false,
    ...prefs,
  };
  userPreferences.set(userId, updated);
  logger.info(`[CrossModel] User ${userId} preferences updated:`, Object.keys(prefs));

  // Sync to user_profile.js (fire-and-forget)
  try {
    const { userProfileManager } = require('./user_profile.js');
    userProfileManager.setUserPreference(userId, {
      model: updated.preferredModel,
      sources: updated.preferredSources,
...
export function getUserPreference(userId) {
  ...
  // Fallback to user_profile.js (sync require)
  try {
    const { userProfileManager } = require('./user_profile.js');
    const profilePrefs = userProfileManager.getUserPreference(userId);
    const prefs = {
      preferredModel: profilePrefs.preferredModel || 'auto',
      preferredSources: profilePrefs.preferredSources || [],
      learningEnabled: profilePrefs.learningEnabled !== false,
    };
    userPreferences.set(userId, prefs); // Cache
    return prefs;
  } catch {
    return { preferredModel: 'auto', preferredSources: [], learningEnabled: true };
  }
}
