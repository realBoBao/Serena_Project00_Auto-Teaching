...
class ImplicitFeedbackManager {
  ...
  async recordClick(linkId, userId) {
    const db = await getDb();
    if (!db) return;
    db.run('UPDATE outbound_links SET clicked = 1, click_at = ? WHERE id = ? AND user_id = ?', [_now(), linkId, userId]);
    const row = db.get('SELECT category FROM outbound_links WHERE id = ?', [linkId]);
    if (row) {
      db.run(`UPDATE category_affinity SET click_count = click_count + 1, last_updated = datetime('now') WHERE user_id = ? AND category = ?`, [userId, row.category]);
    }
  }
  ...
  async recordDwellTime(linkId, userId, dwellTimeMs) {
    const db = await getDb();
    if (!db) return;
    db.run('UPDATE outbound_links SET replied = 1, reply_at = ?, dwell_time_ms = ?, reply_latency_ms = ? WHERE id = ? AND user_id = ?', [_now(), dwellTimeMs, dwellTimeMs, linkId, userId]);
    db.run(`INSERT INTO dwell_time_log (user_id, link_id, dwell_time_ms, category) SELECT ?, ?, ?, category FROM outbound_links WHERE id = ?`, [userId, linkId, dwellTimeMs, linkId]);
    const row = db.get('SELECT category FROM outbound_links WHERE id = ?', [linkId]);
  ...
  async recordSkip(userId, category) {
    const db = await getDb();
    if (!db) return;
    db.run(`UPDATE category_affinity SET skip_count = skip_count + 1, last_updated = datetime('now') WHERE user_id = ? AND category = ?`, [userId, category]);
  ...
  async _recomputeAffinity(userId, category) {
    ...
    db.run(`UPDATE category_affinity SET implicit_score = ?, last_updated = datetime('now') WHERE user_id = ? AND category = ?`, [score, userId, category]);
  }
  ...
  async cleanup(daysToKeep = 90) {
    const db = await getDb();
    if (!db) return;
    db.run("DELETE FROM outbound_links WHERE sent_at < datetime('now', ?)", [`-${daysToKeep} days`]);
    db.run("DELETE FROM dwell_time_log WHERE recorded_at < datetime('now', ?)", [`-${daysToKeep} days`]);
